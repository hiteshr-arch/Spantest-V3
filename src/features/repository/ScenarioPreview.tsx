...
export function ScenarioPreview({ scenario }: { scenario: Scenario }) {
  ...
  if (!steps.length) {
    return (
      <p className="!text-[#8b87a0] text-[12px] italic py-2">
  ...
  if (isApi) {
    return (
      <div className="overflow-x-auto">
        <table className="repo__preview-table w-full text-[11px] border-collapse">
          <thead>
            <tr>
              {[
                "#",
                ...
                <th
                  key={h}
                  className="text-left px-2 py-[6px] !text-[#8b87a0] bg-[#faf9ff] border-b border-[#f3f0fb] whitespace-nowrap font-semibold"
                >
                  {h}
                </th>
              ))}
