...
function MiniPreview({ scenario }: { scenario: Scenario }) {
  ...
  if (!steps.length) {
    return (
      <p className="!text-[#8b87a0] text-[11px] italic py-1" style={{ fontFamily: "'DM Sans', sans-serif" }}>
  ...
  if (isApi) {
    return (
      <div className="overflow-x-auto">
        <table className="w-full text-[10px] border-collapse" style={{ fontFamily: "'DM Sans', sans-serif" }}>
          <thead>
            <tr>
              {["#", "Method", "Endpoint", "Status"].map((h) => (
                <th key={h} className="text-left px-2 py-[4px] !text-[#8b87a0] bg-[#faf9ff] border-b border-[#f3f0fb] whitespace-nowrap" style={{ fontWeight: 600 }}>{h}</th>
              ))}
            </tr>
          </thead>
          <tbody>
...
function ItemCard({ item, index }: { item: RepoItem; index: number }) {
  ...
  return (
    ...
    >
      ...
      >
        <span
          className="flex items-center justify-center w-[22px] h-[22px] rounded-full bg-[#f3eaff] !text-[#7c3aed] text-[10px] shrink-0"
        ...
        <span
          className="flex-1 min-w-0 truncate !text-[#0f0a1e] text-[12.5px]"
        ...
        <Tag
          className="!text-[9px] !px-[6px] !py-0 !m-0 !leading-[18px]"
          style={{ background: prioColor.bg, color: prioColor.text, borderColor: prioColor.border, fontFamily: "'DM Sans', sans-serif", fontWeight: 600 }}
        >
          {item.priority}
        </Tag>
        <Tag
          className="!text-[9px] !px-[6px] !py-0 !m-0 !leading-[18px]"
          style={{ background: catColor.bg, color: catColor.text, borderColor: catColor.border, fontFamily: "'DM Sans', sans-serif", fontWeight: 600 }}
        >
          {item.category}
        </Tag>
  ...
  return (
    <div className="story-detail flex flex-col h-full">
      ...
      <div className="story-detail__header flex items-center gap-3 px-6 py-4 border-b border-[#f3f0fb] bg-white">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2">
            <span
              className="!text-[#8b87a0] text-[10px] uppercase tracking-[1px]"
          ...
          <h2
            className="!text-[#0f0a1e] text-[15px] mt-[2px] truncate !m-0"
        ...
        <Tooltip title={editLabel}>
          <button
            ...
            className="flex items-center gap-2 h-[34px] px-4 rounded-lg bg-[#7c3aed] border-0 !text-white cursor-pointer hover:bg-[#6d28d9] transition-colors text-[12px]"
            style={{ fontFamily: "'DM Sans', sans-serif", fontWeight: 600 }}
          >
            <EditOutlined />
            Edit in Generator
      ...
      <div className="flex items-center gap-2 px-6 py-3 bg-white border-b border-[#f3f0fb]">
        {availableTypes.map((type) => {
          ...
          return (
            <button
              ...
              className="flex items-center gap-[6px] h-[30px] px-3 rounded-lg border-0 cursor-pointer transition-all text-[11.5px]"
              style={{
            ...
            >
              ...
              {cfg.labelPlural}
              <span
                className="text-[10px] min-w-[16px] text-center rounded-full px-[5px] py-[0px]"
                style={{
        ...
        <span className="ml-auto !text-[#8b87a0] text-[10.5px]" style={{ fontFamily: "'DM Sans', sans-serif" }}>
          Total: <span className="!text-[#0f0a1e]" style={{ fontWeight: 600 }}>{items.length}</span> items from this story
        </span>
      </div>
