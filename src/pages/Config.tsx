  ...
  return (
    <div className="config__section bg-white rounded-xl border border-[#f3f0fb] overflow-hidden">
      <div className="flex items-center gap-2 px-5 py-3 border-b border-[#f3f0fb]">
        ...
        <span className="config__section-title !text-[#0f0a1e] text-[13px]">
          {title}
        </span>
...
function MiniStat({ label, value, color }: { label: string; value: number; color: string }) {
  return (
    <div className="flex flex-col items-center gap-1 flex-1 py-3 rounded-lg bg-[#faf9ff] border border-[#f3f0fb]">
      <span
        className="mini-stat__value text-[24px] leading-[24px]"
        style={{ color }}
      >
        {value}
      </span>
      <span className="mini-stat__label !text-[#8b87a0] text-[10px] uppercase tracking-wide">
        {label}
      </span>
    </div>
  );
      ...
      <div className="config__header flex items-center gap-3 px-6 py-4 bg-white border-b border-[#f3f0fb]">
        <div className="flex flex-col gap-1 flex-1">
          {editingName ? (
            <div className="flex items-center gap-2">
              <Input
                ...
                autoFocus
                style={{ width: 320, height: 34, fontSize: 16 }}
              />
              <button
            ...
            <div className="flex items-center gap-2">
              <h1 className="config__project-name !text-[#0f0a1e] text-[18px] m-0">
                {config.name}
              </h1>
          ...
          <div className="flex items-center gap-4">
            <span className="config__meta-item flex items-center gap-1 !text-[#8b87a0] text-[11px]">
          ...
          <div className="flex-1 flex flex-col gap-5 min-w-0">
            ...
            <Section title="Project Details" icon={<FileTextOutlined />}>
              <div className="mb-4">
                ...
                {editingDesc ? (
                  ...
                  <p className="!text-[#4c4568] text-[12px] !leading-[1.7] m-0">
                    {config.description}
                  </p>
                )}
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <span className="!text-[#8b87a0] text-[10px] uppercase tracking-wide font-semibold block mb-1">
                    Status
                  </span>
                  <Select
                    value={config.status}
            ...
            >
              {config.attachments.length === 0 ? (
                <p className="!text-[#8b87a0] text-[12px] italic text-center py-4 m-0">
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          <span className="!text-[#0f0a1e] text-[12px] block truncate">
                            {att.name}
                          </span>
                ...
                <div className="flex flex-col gap-0">
                  {config.activityLog.map((entry, i) => (
                    <div key={entry.id} className="flex gap-3 py-[8px] border-b border-[#f3f0fb] last:border-b-0">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2">
                          <span className="!text-[#0f0a1e] text-[11px] font-semibold">
                            {entry.action}
                          </span>
                          <span className="!text-[#8b87a0] text-[10px]">by {entry.user}</span>
                        </div>
                        <p className="!text-[#4c4568] text-[10.5px] m-0 mt-[2px] !leading-[1.4]">
                          {entry.detail}
                        </p>
                        <span className="!text-[#b0adbe] text-[9px] flex items-center gap-1 mt-[2px]">
